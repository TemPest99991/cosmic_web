...
def load_dashboard_data(use_sample_fallback: bool = True) -> DashboardData:
    ...
    shower = _read_csv("shower_results.csv")
...
def sample_dashboard_data() -> DashboardData:
    ...
    shower_rows = []
    for idx, timestamp in enumerate(times):
        two_fold = max(0, int(5 + 2 * np.sin(idx / 3)))
        three_fold = max(0, int(2 + np.sin(idx / 5)))
        four_fold = 1 if idx % 9 == 0 else 0
        for multiplicity, observed, expected in (
            (2, two_fold, 0.12),
            (3, three_fold, 0.01),
            (4, four_fold, 0.001),
        ):
            shower_rows.append(
                {
                    ...
                    "rate_per_min": observed / 5,
                    "random_expected_count": expected,
                }
            )
    shower = pd.DataFrame(shower_rows)
